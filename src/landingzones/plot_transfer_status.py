...
def load_transfers_for_reporting(
    config_file=None,
    transfers_file=None,
    system=None,
    runtime_ids=None,
...
def infer_report_system(config_file=None, transfers_file=None, runtime_ids=None):
    ...
    try:
        ...
        for system in systems:
            if str(system).lower() in hostname:
                return system
        if runtime_ids and len(systems) == 1:
            return systems[0]
    except Exception:
        pass
    return hostname
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Exact runtime_id to report. Defaults to the runtime IDs generated by the most recent build when available.",
    )
    parser.add_argument(
        "--warning-hours",
        type=float,
