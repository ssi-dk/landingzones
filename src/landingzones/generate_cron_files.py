...
def runtime_filter_metadata_path(crontab_dir):
    """Return the metadata path recording generated runtime_id values."""
    return os.path.join(
        os.path.dirname(os.path.normpath(crontab_dir)),
        RUNTIME_FILTER_METADATA,
    )


def write_runtime_filter_metadata(crontab_dir, runtime_ids):
    """Record the runtime_id values represented by generated artifacts."""
    metadata_path = runtime_filter_metadata_path(crontab_dir)
    ...
    with open(metadata_path, 'w') as handle:
        handle.write("# Runtime IDs generated by landingzones build.\n")
        for value in values:
            handle.write("{0}\n".format(value))
...
def main(argv=None):
    ...
    write_runtime_filter_metadata(
        output_dir,
        transfers_df['runtime_id'].dropna().unique(),
    )
